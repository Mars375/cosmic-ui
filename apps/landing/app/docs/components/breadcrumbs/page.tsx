...
export default function BreadcrumbsPage() {
  ...
  const breadcrumbItemsWithCustomSeparator = [
    { label: 'Home', href: '/' },
    ...
    <div className="min-h-screen bg-cosmic-background text-cosmic-foreground">
      <div className="container mx-auto px-4 py-8">
        ...
        <p className="text-lg text-cosmic-muted mb-8">
          Le composant Breadcrumbs affiche un fil d'Ariane pour la navigation hiérarchique, 
          permettant aux utilisateurs de comprendre leur position dans l'application et de naviguer facilement.
        </p>
...
const items = [
  ...
  { label: 'Page actuelle' } // Pas de href = page actuelle
];
...
<Breadcrumbs items={items} />`}
        ...
        <div className="mb-12">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-4">
              <h3 className="text-lg font-medium">Séparateur personnalisé</h3>
              <div className="flex bg-cosmic-surface rounded-lg p-1">
                <button
                  onClick={() => setShowCode(false)}
                  className={`px-3 py-1 rounded text-sm transition-colors ${
            ...
            <div className="w-[500px] h-[450px] border border-cosmic-border rounded-lg bg-cosmic-surface p-2 flex justify-start">
              {!showCode ? (
                <div className="p-4">
                  <Breadcrumbs items={breadcrumbItemsWithCustomSeparator} separator="→" />
                </div>
              ) : (
                <div className="w-full h-full bg-white dark:bg-black p-2 rounded">
                  <CodeBlock onCopy={() => copyToClipboard(`import { Breadcrumbs } from '@cosmic-ui/ui';
